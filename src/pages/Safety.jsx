        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem' }} className="safety-grid">
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
              ...
              <p className="page-body" style={{ fontSize: '0.95rem', lineHeight: 1.7, color: 'rgba(240,240,250,0.6)', marginBottom: '3rem' }}>
              ...
              {/* Certifications */}
              <div className="page-card-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1px', background: 'rgba(240,240,250,0.06)' }}>
                {['ISNetworld', 'Avetta', 'Veriforce'].map((cert) => (
                  <div key={cert} className="page-card" style={{ background: '#000', padding: '1.5rem', textAlign: 'center' }}>
                    <Award size={20} color="#F5A623" style={{ marginBottom: '0.5rem' }} />
                    <div className="page-accent-title" style={{ fontFamily: 'Barlow Condensed', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--amber)' }}>Grade A</div>
                    <div className="card-body" style={{ fontSize: '0.8rem', color: 'rgba(240,240,250,0.6)', marginTop: '0.2rem' }}>{cert}</div>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 }} style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'rgba(240,240,250,0.06)' }}>
              {[
                ...
                <div key={idx} className="page-card" style={{ background: '#000', padding: '2rem', display: 'flex', gap: '1rem' }}>
                  ...
                  <div>
                    <h4 className="card-title" style={{ fontFamily: 'Inter', fontSize: '0.95rem', fontWeight: 600, color: '#f0f0fa', marginBottom: '0.4rem' }}>{p.title}</h4>
                    <p className="card-body" style={{ fontSize: '0.85rem', lineHeight: 1.6, color: 'rgba(240,240,250,0.6)' }}>{p.desc}</p>
                  </div>
                </div>
              ))}
            </motion.div>
